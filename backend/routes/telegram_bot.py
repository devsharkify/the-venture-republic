...
async def generate_morning_report():
    ...
    return f"""<b>Good Morning! The Venture Republic Daily Report</b>
<b>Date:</b> {today} (IST)
...
async def generate_evening_report():
    ...
    return f"""<b>Good Evening! The Venture Republic Day Summary</b>
<b>Date:</b> {today}
...
async def send_epaper_pdf(date, slot, lang):
    ...
    try:
        ...
        caption = f"The Venture Republic ePaper - {date} {edition} ({lang.upper()})"
...
async def send_error_alert(error_msg):
    ...
    if not chat_id:
        return
    text = f"<b>ALERT: The Venture Republic Error</b>\n\n{error_msg}\n\n<i>{_ist_now().strftime('%I:%M %p IST')}</i>"
