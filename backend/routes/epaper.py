...
async def get_epaper_edition(date: str, lang: str = Query("en", regex="^(en|te)$"), slot: str = Query("", regex="^(morning|evening|)$")):
    ...
    if not all_articles:
        return {"date": date, "slot": effective_slot, "lang": lang,
                "edition_title": f"The Venture Republic - {get_edition_label(effective_slot, lang)}",
...
async def get_epaper_pdf(date: str, lang: str = Query("en", regex="^(en|te)$"), slot: str = Query("", regex="^(morning|evening|)$")):
    ...
    def _cmyk_footer(pn, tp):
        ...
        return f'<div style="margin-top:10px;padding-top:6px;border-top:2px solid #1a1a1a;display:flex;justify-content:space-between;align-items:center"><div>{dl}</div><div style="font-size:7px;color:#999">theventurerepublic.in \u2022 Page {pn}/{tp} \u2022 {date}</div><div>{dr}</div></div>'
...
<span>theventurerepublic.in</span>
</div></div>'''
        else:
